...
class FluxFlowSlider(FluxBase):
    def sample(self,
        ...
        pbar = tqdm(enumerate(timesteps), total=NFE, desc="FLUX FlowSlider")
        for i, t in pbar:
            if i < n_start:
                continue
            ...
            with torch.no_grad():
                v_qt_src = self.predict_vector(qt, t_tensor, src_emb, src_pool, src_txt_ids, img_ids, guidance=guidance)
                v_pt_src = self.predict_vector(pt, t_tensor, src_emb, src_pool, src_txt_ids, img_ids, guidance=guidance)
                v_pt_tar = self.predict_vector(pt, t_tensor, tgt_emb, tgt_pool, tgt_txt_ids, img_ids, guidance=guidance)
