...
class FluxTweezeEdit(FluxBase):
    def sample(self,
        ...
        tgt_guidance = torch.tensor([tgt_cfg_scale], device=device).expand(z_src.shape[0])

        pbar = tqdm(enumerate(timesteps), total=NFE, desc="FLUX TweezeEdit")
        for i, t in pbar:
            t_curr = t
            ...
            with torch.no_grad():
                ...
                v_tar = self.predict_vector(pt, t_tensor, tgt_emb, tgt_pool, tgt_txt_ids, img_ids, guidance=tgt_guidance)

            z0_hat_src = qt - t_curr * v_src
            z0_hat_tar = pt - t_curr * v_tar

            # gamma_hat in [-|dt|, 0]; scaled by user gamma in [0,1]
